  ...
  return (
    <div className="order-summary">
      <div className="order-summary__header">
        ...
        <button className="order-summary__add" onClick={() => navigate('/')}>
          Add Items
        </button>
      </div>
      ...
      <div className="order-summary__items">
        {state.items.map((ci) => (
          <CartItemRow key={ci.item.id} cartItem={ci} onEdit={() => navigate('/')} />
        ))}
      </div>
      ...
      <div className="order-summary__fees">
        ...
        {priorityFee > 0 && (
          <div className="fee-row">
            ...
            <span>{formatRupiah(priorityFee)}</span>
          </div>
        )}
        {deliveryDiscount < 0 && (
          <div className="fee-row fee-row--discount">
            <span>Delivery disc 10rb min 40rb</span>
            <span>{formatRupiah(deliveryDiscount)}</span>
          </div>
        )}
      </div>
      ...
      <div className="order-summary__section">
        ...
        <div className="eco-row">
          ...
          <Toggle
            checked={state.greenContribution}
            onChange={() => dispatch({ type: 'TOGGLE_GREEN' })}
          />
        </div>
      </div>
      ...
      <div className="order-summary__section">
        <div className="cutlery-row">
          ...
          <Checkbox
            checked={state.cutlery}
            onChange={() => dispatch({ type: 'TOGGLE_CUTLERY' })}
          />
        </div>
      </div>
      ...
      <div className="order-summary__total">
        <div className="total-row">
          ...
          <div className="total-row__right">
            ...
            {deliveryDiscount < 0 && (
              <span className="total-row__original">{formatRupiah(originalTotal)}</span>
            )}
          </div>
        </div>
        {deliveryDiscount < 0 && (
          <p className="total-row__saved">
